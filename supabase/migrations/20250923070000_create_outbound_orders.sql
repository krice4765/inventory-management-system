-- 出庫管理テーブルの作成
-- Create outbound_orders table and related structures
...
-- 出庫管理番号自動生成関数
CREATE OR REPLACE FUNCTION generate_outbound_order_number()
RETURNS TEXT AS $$
DECLARE
...
BEGIN
  ...
  SELECT COALESCE(MAX(
    CASE
      WHEN order_number LIKE 'OUT-' || current_date_str || '-%'
  ...
  WHERE order_number LIKE 'OUT-' || current_date_str || '-%';

  -- 番号を3桁0埋めで生成
  order_number := 'OUT-' || current_date_str || '-' || LPAD(sequence_num::TEXT, 3, '0');
...
-- 出庫管理番号自動設定トリガー
CREATE OR REPLACE FUNCTION set_outbound_order_number()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.order_number IS NULL OR NEW.order_number = '' THEN
    NEW.order_number := generate_outbound_order_number();
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trigger_set_outbound_order_number ON outbound_orders;
CREATE TRIGGER trigger_set_outbound_order_number
  BEFORE INSERT ON outbound_orders
  FOR EACH ROW
  EXECUTE FUNCTION set_outbound_order_number();
...
-- コメント追加
COMMENT ON TABLE outbound_orders IS '出庫管理テーブル - 出庫指示と出荷管理';
COMMENT ON TABLE outbound_order_items IS '出庫指示明細テーブル - 出庫する商品の詳細';
COMMENT ON COLUMN outbound_orders.order_number IS '出庫管理番号（自動生成）';
COMMENT ON COLUMN outbound_orders.customer_name IS '出荷先顧客名';
COMMENT ON COLUMN outbound_orders.status IS '出庫ステータス: pending(出庫待ち), processing(処理中), completed(完了), cancelled(キャンセル)';
COMMENT ON COLUMN outbound_order_items.quantity_requested IS '要求数量';
COMMENT ON COLUMN outbound_order_items.quantity_shipped IS '出荷済み数量';