...
-- =====================================================
-- 4. inventory_movementsテーブル作成（在庫移動履歴）
-- =====================================================
...
COMMENT ON COLUMN orders.order_number IS '注文番号（自動生成）';
COMMENT ON COLUMN orders.status IS '注文ステータス: pending(保留), confirmed(確定), processing(処理中), shipped(出荷済), delivered(配送完了), cancelled(キャンセル)';
