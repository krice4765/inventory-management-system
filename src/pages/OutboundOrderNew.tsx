...
interface OutboundOrderItem {
      product_id: string; quantity_requested: number; unit_price_tax_excluded: number; }

interface OutboundFormData {
      customer_name: string; request_date: string; due_date: string; notes: string; }
...
export default function OutboundOrderNew() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const handleCustomerChange = (customerId: string) => { setSelectedCustomer(customerId);
  ...
  const getTotalAmount = () => {
    return items.reduce((sum, item) => {
      ...
      const taxRate = product?.tax_category === 'reduced_8' ? 0.08 : 0.10; const unitPriceTaxIncluded = Math.floor(item.unit_price_tax_excluded * (1 + taxRate));
    ...
    try {
      const request: CreateOutboundRequest = { customer_name: formData.customer_name,
      ...
      toast.error(error instanceof Error ? error.message : '出庫指示の作成に失敗しました'); } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
      ...
      <h1 className="text-3xl font-bold text-gray-900 dark: text-white">新規出庫指示作成</h1> </div>
          <button
      ...
      <h2 className="text-lg font-semibold text-gray-900 dark: text-white">出庫指示情報</h2> </div>
          </div>

          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            {/* ヘッダー情報 */}
      <div className="grid grid-cols-1 md: grid-cols-2 gap-6"><div>
      <label className="block text-sm font-medium text-gray-700 dark: text-gray-300 mb-1">顧客 <span className="text-red-500">*</span>
                </label>
                <SearchableSelect
              ...
              <div>
      <label className="block text-sm font-medium text-gray-700 dark: text-gray-300">出庫依頼日
                </label>
                <input
                  type="date"
                  required
      className="mt-1 block w-full border border-gray-300 dark: border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500"value={formData.request_date}
                  onChange={(e) => setFormData({ ...formData, request_date: e.target.value })}
                />
              </div>

              <div>
      <label className="block text-sm font-medium text-gray-700 dark: text-gray-300">希望納期 <span className="text-red-500">*</span>
                </label>
                <input
                  type="date"
                  required
      className="mt-1 block w-full border border-gray-300 dark: border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500"value={formData.due_date}
                  onChange={(e) => setFormData({ ...formData, due_date: e.target.value })}
                />
              </div>

              <div>
      <label className="block text-sm font-medium text-gray-700 dark: text-gray-300">備考
                </label>
                <input
                  type="text"
      className="mt-1 block w-full border border-gray-300 dark: border-gray-600 rounded-md px-3 py-2 bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:outline-none focus:ring-blue-500 focus:border-blue-500"value={formData.notes}
      ...
      <div className="mt-2 text-xs text-gray-600 dark: text-gray-400 bg-green-50 dark:bg-green-900/20 p-2 rounded border border-green-200 dark:border-green-700"><div className="font-medium text-green-900 dark:text-green-100">{selectedProduct.product_name}</div>
      ...
      <div className="lg: col-span-2"><label className="block text-sm font-medium text-gray-700 dark: text-gray-300 mb-2">数量 <span className="text-red-500">*</span>
                        ...
                        <input
                          ...
                          min="1"
                          step="1"
      className="w-full border border-gray-300 dark: border-gray-600 rounded-md px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500"value={isNaN(item.quantity_requested) ? '' : item.quantity_requested}
                          onChange={(e) => {
                          ...
                          onBlur={(e) => {
                            const value = parseInt(e.target.value, 10);
      const validatedQuantity = Math.max(1, isNaN(value) ? 1 : Math.floor(value)); if (validatedQuantity !== item.quantity_requested) {
                      ...
                      {/* 単価エリア */}
      <div className="lg: col-span-2"><label className="block text-sm font-medium text-gray-700 dark: text-gray-300 mb-2">単価（税抜）<span className="text-red-500">*</span>
                        </label>
                        <input
                          type="number"
                          step="0.01"
                          min="0"
      className="w-full border border-gray-300 dark: border-gray-600 rounded-md px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-blue-500 focus:border-blue-500"value={isNaN(item.unit_price_tax_excluded) ? '' : item.unit_price_tax_excluded}
                          onChange={(e) => {
                          ...
                          onBlur={(e) => {
                            const value = parseFloat(e.target.value);
      const validatedPrice = Math.max(0, isNaN(value) ? 0 : value); if (validatedPrice !== item.unit_price_tax_excluded) {
                      ...
                      {/* 合計金額エリア */}
      <div className="lg: col-span-2"><label className="block text-sm font-medium text-gray-700 dark: text-gray-300 mb-2">合計金額（税込）
                        </label>
      ...
      className="w-full bg-red-50 dark: bg-red-900 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-800 border border-red-200 dark:border-red-700 rounded-md px-3 py-2 text-sm font-medium transition-colors"title="この明細行を削除"
              ...
              {/* 合計計算 */}
      <div className="mt-6 bg-gray-50 dark: bg-gray-700 p-4 rounded-lg"><div className="text-right space-y-2 text-gray-900 dark: text-white"><div className="flex justify-between text-lg font-bold border-t border-gray-300 dark: border-gray-600 pt-2"><span>合計金額（税込）: </span> <span>¥{getTotalAmount().toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
