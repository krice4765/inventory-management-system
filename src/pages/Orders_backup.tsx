...
const fetchOrders = async () => {
  ...
  const deliveryProgressData = await Promise.all(
      (purchaseOrders || []).map(async (order: any) => { // 分納実績を集計（重複排除とソート追加）
      ...
      let progress_status: string; const tolerance = 1; // 1円の誤差許容
      ...
      // 個数指定分納の場合: 金額と個数の両方をチェック if (amountComplete && delivered_amount > 0) {
      // ⚡ TODO: 将来的には在庫移動データから個数完了状況を確認 // 現在は金額完了+個数指定がある場合は納品完了とする
      ...
      // 金額のみの分納の場合: 従来通り if (amountComplete) {
          progress_status = '納品完了';
        } else if (delivered_amount > 0) {
          progress_status = '一部納品';
        } else {
      ...
      deliveries_detail: deliveries?.map(d => ({ id: d.id,
...
export default function Orders() {
  ...
  const { data: orders, isLoading, isError, error, refetch, isFetching } = useQuery<DeliveryProgress[], Error>({
    ...
    refetchIntervalInBackground: false, // バックグラウンドでは停止
      onSuccess: (data) => { },
      onError: (err) => { console.error('発注データ取得エラー:', err);
  ...
  const filteredOrders = useMemo(() => {
    ...
    filtered.sort((a: any, b: any) => {
      ...
      valueA = a.delivery_deadline ? new Date(a.delivery_deadline).getTime() : 0; valueB = b.delivery_deadline ? new Date(b.delivery_deadline).getTime() : 0; break;
        case 'partner_name':
  ...
  const clearFilters = () => {
    ...
    setSortOrder('desc');
      ...
      const handleSort = (field: 'created_at' | 'delivery_deadline' | 'partner_name') => { if (sortBy === field) {
      ...
      const getSortIcon = (field: 'created_at' | 'delivery_deadline' | 'partner_name') => { if (sortBy !== field) return '↕️';
      return sortOrder === 'asc' ? '↑' : '↓'; };

      const getProgressColor = (status: string) => { switch (status) {
      case '未納品': return 'bg-red-100 text-red-800'; case '一部納品': return 'bg-yellow-100 text-yellow-800'; case '納品完了': return 'bg-green-100 text-green-800'; case '要確認': return 'bg-orange-100 text-orange-800'; default: return 'bg-gray-100 text-gray-800'; }
  };
  ...
  // PDF生成処理
      const handleGeneratePDF = async (order: DeliveryProgress) => { try {
      ...
      const pdfData: OrderPDFData = { id: orderDetail.id,
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 dark: from-gray-900 dark:via-blue-900/20 dark:to-purple-900/20 transition-all duration-500"><div className="flex items-center justify-center h-64">
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
            className="rounded-full h-16 w-16 border-4 border-blue-500 border-t-transparent"
          />
      <span className="ml-3 text-gray-700 dark: text-gray-300 font-medium">発注データを読み込み中...</span> </div>
      </div>
    );
  }
  ...
  return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 via-purple-50 to-pink-50 dark: from-gray-900 dark:via-blue-900/20 dark:to-purple-900/20 transition-all duration-500"><div className="p-6 space-y-6">
        <motion.div 
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                {isFetching && (
      <div className="flex items-center gap-2 px-3 py-1 bg-blue-100 dark: bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full text-sm"><div className="w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                    更新中
                  </div>
                )}
              </div>
      <p className="text-sm text-gray-600 dark: text-gray-400">発注・分納・納期管理</p> </div>
          </div>
          <div className="flex items-center gap-3">
            <motion.button 
              onClick={() => refetch()} 
              disabled={isFetching} 
      className="flex items-center px-4 py-2 text-sm bg-white/80 dark: bg-gray-800/80 backdrop-blur-md border border-gray-200/50 dark:border-gray-700/50 text-gray-700 dark:text-gray-300 rounded-xl hover:bg-white dark:hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-xl"whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
      ...
      className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-xl hover: from-green-600 hover:to-green-700 transition-all shadow-lg hover:shadow-xl font-medium text-sm"whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
      ...
      <div className="flex flex-col sm: flex-row gap-4 items-center">{/* 検索バー */}
      ...
      className="w-full pl-12 pr-4 py-3 border-2 border-gray-200 dark: border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all placeholder-gray-500 dark:placeholder-gray-400"value={searchTerm}
      ...
      <div className="p-1.5 bg-gray-100 dark: bg-gray-700 rounded-lg"><Filter className="h-4 w-4 text-gray-500 dark: text-gray-400" /></div>
      <label className="text-sm font-semibold text-gray-700 dark: text-gray-300 whitespace-nowrap">状態:</label> <select
      className="px-4 py-2 border-2 border-gray-200 dark: border-gray-600 rounded-xl bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"value={statusFilter}
      ...
      className="inline-flex items-center px-4 py-2 border-2 border-gray-200 dark: border-gray-600 rounded-xl text-sm font-semibold text-gray-700 dark:text-gray-300 bg-gray-50 dark:bg-gray-700 hover:bg-red-50 dark:hover:bg-red-900/20 hover:border-red-300 dark:hover:border-red-600 hover:text-red-600 dark:hover:text-red-400 transition-all"whileHover={{ scale: 1.05 }}
          ...
          {/* 検索結果数表示 */}
      <div className="mt-4 pt-4 border-t border-gray-200 dark: border-gray-700"><div className="flex flex-wrap items-center gap-2 text-sm">
              {searchTerm || statusFilter !== 'all' ? (
                <>
      <span className="font-medium text-gray-900 dark: text-white">{filteredOrders.length}件の結果
                  </span>
      <span className="text-gray-500 dark: text-gray-400">(全{orders?.length || 0}件中)
                  </span>
                  {searchTerm && (
      <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-blue-100 dark: bg-blue-900/30 text-blue-800 dark:text-blue-200">「{searchTerm}」で検索中
                  ...
                  {statusFilter !== 'all' && (
      <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-100 dark: bg-purple-900/30 text-purple-800 dark:text-purple-200">{statusFilter}でフィルタ中
                    </span>
                  )}
                </>
              ...
              {searchTerm && (
      <div className="w-full mt-2 text-xs text-gray-500 dark: text-gray-500 bg-blue-50 dark:bg-blue-900/10 px-3 py-2 rounded-lg">💡 発注番号（PO250910004）、発注ID（UUID）、仕入先名で検索可能
                </div>
              )}
            </div>
          </div>
      ...
      <div className="px-6 py-4 bg-gradient-to-r from-gray-50 to-gray-100 dark: from-gray-800 dark:to-gray-700 border-b border-gray-200/50 dark:border-gray-700/50"><h2 className="text-xl font-bold text-gray-900 dark: text-white flex items-center gap-2"><FileText className="w-6 h-6 text-blue-600 dark: text-blue-400" />発注一覧
            </h2>
          </div>
      ...
      className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark: text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 select-none"onClick={() => handleSort('created_at')}
                ...
                <th 
      className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark: text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 select-none"onClick={() => handleSort('partner_name')}
                >
                  <div className="flex items-center space-x-1">
                    <span>仕入先</span>
                ...
                <th 
      className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark: text-gray-400 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 select-none"onClick={() => handleSort('delivery_deadline')}
                >
                  <div className="flex items-center space-x-1">
                    <span>納期</span>
      ...
      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark: text-gray-400 uppercase tracking-wider">金額・進捗
                </th>
      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark: text-gray-400 uppercase tracking-wider">ステータス
                </th>
      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark: text-gray-400 uppercase tracking-wider">操作
                </th>
              </tr>
            </thead>
      <tbody className="bg-white dark: bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">{filteredOrders.map((order) => {
                const progressPercentage = getProgressPercentage(order.delivered_amount, order.ordered_amount);
                return (
      console.log("Debug:", { <tr key={order.purchase_order_id} className="hover: bg-gray-50 dark:hover:bg-gray-800 transition-colors"> });
      ...
      <div className="text-sm text-gray-500 dark: text-gray-400"><Calendar className="inline w-4 h-4 mr-1" />
      ...
      <div className="text-sm text-gray-900 dark: text-white"><Calendar className="inline w-4 h-4 mr-1 text-orange-500" />
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
      <div className="text-sm text-gray-900 dark: text-white"><div>発注額: ¥{order.ordered_amount.toLocaleString()}</div>
      ...
      className="inline-flex items-center px-3 py-1.5 bg-green-50 dark: bg-green-900/20 text-green-600 dark:text-green-400 hover:bg-green-100 dark:hover:bg-green-900/40 hover:text-green-800 dark:hover:text-green-300 font-medium rounded-lg transition-all text-xs"whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                          >
                            分納入力
                          </motion.button>
      ) : ( <span className="inline-flex items-center px-3 py-1.5 bg-gray-50 dark: bg-gray-800 text-gray-400 dark:text-gray-500 rounded-lg text-xs">完了済み
                          </span>
                        )}
                        <motion.button 
                          onClick={(e) => {
      ...
      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 dark: bg-blue-900/20 text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/40 hover:text-blue-800 dark:hover:text-blue-300 font-medium rounded-lg transition-all text-xs"whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
      ...
      <h3 className="mt-2 text-sm font-medium text-gray-900 dark: text-white">{searchTerm || statusFilter !== 'all' ? '該当する発注が見つかりません' : '発注がありません'}
              </h3>
      <p className="mt-1 text-sm text-gray-500 dark: text-gray-400">{searchTerm || statusFilter !== 'all' 
                  ? '検索条件を変更してお試しください' 
      : '新しい発注を作成してください' }
              </p>
              {!(searchTerm || statusFilter !== 'all') && (
