...
interface Props {
      parentOrderId: string; onSuccess?: () => void; }
...
export const InstallmentIntegrationManager: React.FC<Props> = ({
  ...
  const createMutation = useMutation({
    mutationFn: (params: IntegratedInstallmentParams) => createIntegratedInstallment(params),
      onSuccess: (result) => { if (result.integrationStatus === 'success') {
  ...
  const repairMutation = useMutation({
    mutationFn: ({ transactionId, products }: {
      transactionId: string; products: Array<{ productId: string; quantity: number; unitPrice: number; }>;
  ...
  const handleSubmit = () => {
    if (products.length === 0) {
      ...
      const params: IntegratedInstallmentParams = { parentOrderId,
      amount,
      products,
      memo,
    };
  ...
  return (
    <div className="bg-white p-6 rounded-lg border shadow-sm">
      ...
      ) : validation ? ( <div className={`p-3 rounded ${validation.isValid ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border`}>
                ...
                {validation.issues.map((issue, index) => (
                  <div key={index} className="text-sm text-red-700 flex items-center justify-between">
                    ...
                    {issue.type === 'missing_inventory' && issue.transactionId && (
                      <button
                        onClick={() => {
      // 簡単な修復: 空の商品配列で修復実行 repairMutation.mutate({
                            transactionId: issue.transactionId!,
                            products: [{ productId: 'sample-product', quantity: 1, unitPrice: 1000 }]
                          });
                        }}
      className="ml-2 px-2 py-1 bg-yellow-500 text-white rounded text-xs hover: bg-yellow-600"disabled={repairMutation.isPending}
                      >
                        🔧 修復
                      </button>
                    )}
      ...
      className="w-full px-3 py-2 border rounded focus: ring-2 focus:ring-blue-500"placeholder="金額を入力"
            />
          </div>
          <div>
      ...
      className="w-full px-3 py-2 border rounded focus: ring-2 focus:ring-blue-500"placeholder="第○回など"
            />
          </div>
        </div>
