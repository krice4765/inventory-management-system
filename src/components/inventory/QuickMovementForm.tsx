...
export const QuickMovementForm: React.FC<Props> = ({ mode, onSuccess }) => {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      const movement_type: MovementType = mode === 'in' ? 'purchase' : 'sale';
      const quantity_delta = mode === 'in' ? Math.abs(values.quantity) : -Math.abs(values.quantity);
      ...
      const actionName = mode === 'in' ? '入庫' : '出庫';
      toast.success(`${actionName}を登録しました`);
      reset();
      onSuccess?.();
    } catch (error: Error) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        >
          ...
          {products.map(product => (
            <option key={product.id} value={product.id}>
              {product.name} ({product.product_code}) - 在庫: {product.stock_quantity}
            </option>
          ))}
        </select>
      ...
      <div>
        ...
        <input
          type="text"
          className="w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
          placeholder={mode === 'in' ? '入庫の理由や備考' : '出庫の理由や備考'}
          {...register('note')}
        />
      ...
      <div className="pt-4 border-t border-gray-200">
        <button
          ...
          className={`w-full px-4 py-2 rounded-md text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed ${
            mode === 'in'
