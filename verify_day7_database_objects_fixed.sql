-- Day 7-8 データベースオブジェクト実装確認スクリプト（修正版）
...
BEGIN
    ...
    FOREACH trg_name IN ARRAY expected_triggers LOOP
        SELECT
            event_object_table,
        ...
        IF trigger_table IS NOT NULL THEN
            RAISE NOTICE '✅ % → % [%]', trg_name, trigger_table, trigger_event;
            existing_count := existing_count + 1;
        ELSE
            RAISE NOTICE '❌ %', trg_name;
        END IF;
...
BEGIN
    ...
    FOREACH tbl_name IN ARRAY expected_tables LOOP
        ...
        IF index_count > 0 THEN
            RAISE NOTICE '✅ %: % 個のインデックス', tbl_name, index_count;
        ELSE
            RAISE NOTICE '❌ %: インデックスなし', tbl_name;
        END IF;
    END LOOP;
...
BEGIN
    ...
    FOREACH tbl_name IN ARRAY expected_tables LOOP
        ...
        IF column_count > 0 THEN
            RAISE NOTICE '✅ % (% カラム)', tbl_name, column_count;
        ELSE
            RAISE NOTICE '❌ % (テーブル未作成)', tbl_name;
        END IF;
    END LOOP;
...
BEGIN
    RAISE NOTICE '';
    RAISE NOTICE '============================================';
    RAISE NOTICE '🔐 RLS (Row Level Security) 設定確認';
    RAISE NOTICE '============================================';
    ...
    FOREACH tbl_name IN ARRAY expected_tables LOOP
        ...
        IF rls_enabled IS NOT NULL THEN
            RAISE NOTICE '%s % %s (% ポリシー)',
                CASE WHEN rls_enabled THEN '✅' ELSE '❌' END,
                tbl_name,
                CASE WHEN rls_enabled THEN 'RLS有効' ELSE 'RLS無効' END,
                policy_count;
        ELSE
            RAISE NOTICE '❌ % (テーブル未作成)', tbl_name;
        END IF;
